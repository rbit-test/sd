...
RETRY_DELAY_INITIAL = 5          # Initial retry delay for secondary rate limits (seconds)
REQUEST_TIMEOUT = 30             # HTTP request timeout in seconds
MAX_CONCURRENT_REQUESTS = 5      # Maximum concurrent HTTP requests (reduced for rate limiting)
...
def validate_github_token(token):
    ...
    if not token:
        return False
    # GitHub tokens should be at least 20 characters and start with specific prefixes
    if len(token) < 20:
        return False
    # Classic tokens start with 'ghp_', GitHub App tokens with 'ghs_', etc.
    valid_prefixes = ['ghp_', 'ghs_', 'gho_', 'ghu_', 'github_pat_']
    if not any(token.startswith(prefix) for prefix in valid_prefixes):
        # Allow for older tokens that might not have prefixes
        if not token.replace('_', '').isalnum():
            return False
    return True

def safe_token_display(token):
    """Safely display token with proper masking."""
...
async def check_rate_limit_status(session, instance):
    """Check current rate limit status before starting search."""
    try:
        if instance == "cloud":
            rate_limit_url = "https://api.github.com/rate_limit"
        else:
            # For on-premise, rate limit endpoint might be different
            rate_limit_url = "https://api.github.com/rate_limit"  # Adjust as needed
        ...
        async with session.get(rate_limit_url) as resp:
            if resp.status == 200:
                ...
                if search_rate:
                    ...
                    print_info(f"Rate Limit Status: {remaining}/{limit} search requests remaining")
                    if remaining < 10:
                        reset_datetime = datetime.fromtimestamp(reset_time)
                        print_warning(f"Low rate limit remaining! Resets at: {reset_datetime.strftime('%H:%M:%S')}")
                        if remaining == 0:
                            wait_time = reset_time - int(time.time())
                            print_error(f"Rate limit exhausted! Must wait {wait_time} seconds")
                            return False
                    return True
            else:
                print_warning("Could not check rate limit status")
                return True
    except Exception as e:
        print_warning(f"Rate limit check failed: {e}")
    ...
    if not token:
        ...
        while True:
            ...
            if validate_github_token(token):
                break
            print_error("Invalid GitHub token format. Please enter a valid token.")
        ...
        if validate_github_token(token):
            print_success(f"Using GitHub token from .env file: {safe_token_display(token)}")
        else:
            print_warning("Token from .env file appears invalid. Please check your token.")
            while True:
                token = ask_required("Enter a valid GitHub Personal Access Token", "GitHub Token")
                if validate_github_token(token):
                    break
                print_error("Invalid GitHub token format. Please enter a valid token.")
...
# Github Search with improved error handling
async def search_github_code(session, api_url, query, max_results=MAX_SEARCH_RESULTS, repo_scope=None):
    ...
    while len(all_files) < max_results:
        ...
        try:
            async with session.get(api_url, params=params) as resp:
                if resp.status in [403, 429]:  # Rate limit hit (both primary and secondary)
                    # Check for different types of rate limit headers
                    reset_time = None
                    retry_after = None
                    
                    # GitHub uses different headers for different rate limits
                    if 'X-RateLimit-Reset' in resp.headers:
                        reset_time = int(resp.headers.get("X-RateLimit-Reset"))
                    if 'Retry-After' in resp.headers:
                        retry_after = int(resp.headers.get("Retry-After"))
                    
                    # Calculate wait time based on available headers
                    if retry_after:
                        wait = retry_after
                        rate_type = "Secondary rate limit"
                    elif reset_time:
                        wait = max(reset_time - int(time.time()), RETRY_DELAY_INITIAL)
                        rate_type = "Primary rate limit"
                    else:
                        # Fallback with exponential backoff
                        wait = min(RETRY_DELAY_INITIAL * (2 ** retry_count), 300)  # Max 5 minutes
                        rate_type = "Rate limit (unknown type)"
                    
                    print_warning(f"{rate_type} hit (HTTP {resp.status}), waiting {wait}s...")
                    print_info(f"Page {page}: Processed {len(all_files)}/{max_results} results so far")
                    
                    await asyncio.sleep(wait)
                    retry_count += 1
                    if retry_count > max_retries:
                        print_error("Maximum retries exceeded for rate limiting")
                        print_info(f"Collected {len(all_files)} results before hitting rate limits")
                        break
                    continue
                elif resp.status == 422:  # Unprocessable Entity - likely query too complex
                    print_error("Search query too complex or invalid")
                    break
                elif resp.status >= 500:  # Server errors
                    wait = min(RETRY_DELAY_INITIAL * (2 ** retry_count), 60)  # Exponential backoff
                    print_warning(f"Server error {resp.status}, retrying in {wait}s...")
                    await asyncio.sleep(wait)
                    retry_count += 1
                    if retry_count > max_retries:
                        print_error("Maximum retries exceeded for server errors")
                        break
                    continue
                ...
                # Progress update
                print_info(f"Page {page}: Collected {len(all_files)}/{max_results} results")
                ...
                if page <= 10:  # More conservative for first 10 pages
                    await asyncio.sleep(1)  # 1 second delay
                else:  # Slower for subsequent pages to avoid rate limits
            ...
            retry_count += 1
            if retry_count > max_retries:
                print_error("Maximum retries exceeded for network errors")
                break
            wait = min(RETRY_DELAY_INITIAL * (2 ** retry_count), 60)
            await asyncio.sleep(wait)
            continue
        except Exception as e:
    ...
    return all_files, data.get('total_count', 0) if 'data' in locals() else 0
...
async def main():
    ...
    try:
        ...
        # Record start time for performance tracking (after user inputs)
        ...
        print_info("🔄 Rate limiting strategy: 1-2s delays between requests")
        print_warning("Note: Large searches may take longer due to rate limiting compliance")
        ...
        async with aiohttp.ClientSession(
            ...
            print_section_header("Checking Rate Limit Status")
            can_proceed = await check_rate_limit_status(session, instance)
            if not can_proceed:
                print_error("Cannot proceed due to rate limit exhaustion. Please try again later.")
                return
            ...
            if instance == "cloud":
                ...
                for idx, org in enumerate(ORGANIZATIONS):
                    try:
                        ...
                        print_error(f"Error searching in organization {org}: {e}")
                        continue
                ...
                try:
                    ...
                    print_error(f"Error during on-premise search: {e}")
                    results = []
                    total_count = 0
        ...
        # Step 5: Extract fragments into each item for CSV (with memory optimization)
        processed_results = []
        total_items = len(results)
        chunk_size = 100  # Process in smaller chunks
        
        print_info(f"Processing {total_items} search results in chunks...")
        ...
        for i in range(0, total_items, chunk_size):
            ...
            for item in chunk:
                try:
                    ...
                    processed_item["fragment"] = "\n---\n".join(fragments)  # separate multiple matches by --- 
                    chunk_processed.append(processed_item)
                except Exception as e:
                    print_error(f"Error processing search result: {e}")
                    continue
            
            processed_results.extend(chunk_processed)
            ...
            print_info(f"Processed {progress}/{total_items} results")
        
        print_success(f"Fragment extraction complete: {len(processed_results)} results processed") 
            
        fragments_file = os.path.join(pattern_folder, f"debug_{file_name}_fragments.csv")
        pattern_lines_file = os.path.join(pattern_folder, f"{file_name}_pattern_lines.csv")
        
        # Step 6: Save results to CSV (robust)
        save_results_to_csv(processed_results, gh_code_search_columns, fragments_file)
        ...
        # Update summary file with execution time
        summary_file = os.path.join(pattern_folder, "Search_Summary.txt")
        try:
            with open(summary_file, "a", encoding="utf-8") as f:
                f.write(f"\nExecution Details:\n")
                ...
                f.write(f"Total Execution Time: {time_str}\n")
                f.write(f"Files Processed: {len(processed_results)}\n")
        except Exception as e:
            print_error(f"Error updating summary file with execution time: {e}")
        ...
        print(f"   📊 debug_{file_name}_fragments.csv - Complete search results for debug")
        print(f"   🎯 {file_name}_pattern_lines.csv - Filtered lines containing the search pattern")
        
        print("\n" + "=" * 70)
        print_success(f"⏱️  Total execution time: {time_str}")
        print_info(f"Started: {start_datetime.strftime('%Y-%m-%d %H:%M:%S')}")
        print_info(f"Completed: {end_datetime.strftime('%Y-%m-%d %H:%M:%S')}")
        print("🎉 Thank you for using GitHub Search Automation Tool! 🎉")
        print("=" * 70)
        
    except KeyboardInterrupt:
        # Calculate time even when interrupted
        end_time = time.time()
        execution_time = end_time - start_time
        minutes = int(execution_time // 60)
        seconds = execution_time % 60
        time_str = f"{minutes}m {seconds:.2f}s" if minutes > 0 else f"{seconds:.2f}s"
        print_info(f"\n\nOperation cancelled by user after {time_str}. Goodbye!")
    except Exception as e:
        # Calculate time even when error occurred
        end_time = time.time()
        execution_time = end_time - start_time
        minutes = int(execution_time // 60)
        seconds = execution_time % 60
        time_str = f"{minutes}m {seconds:.2f}s" if minutes > 0 else f"{seconds:.2f}s"
        print_error(f"An unexpected error occurred after {time_str}: {e}")
...
def save_results_to_csv(results, columns, filename="search_results.csv"):
    """Save results to CSV with dynamic columns including nested keys.
       Handles errors gracefully and optimizes memory usage for large datasets.
    """
    ...
    try:
        ...
        with open(filename, "w", newline="", encoding="utf-8") as f:
            ...
            # Process results in chunks to optimize memory usage
            for i in range(0, len(results), chunk_size):
                ...
                for item in chunk:
                    try:
                        ...
                        print_error(f"Error writing record: {e}")
                        continue
                
                # Clear chunk from memory
                del chunk
                
        print_success(f"Results saved to {os.path.basename(filename)} ({record_count} records)")
    except Exception as e:
